...
from models.backbones.mobilemamba_lib.vmambanew import SS2D
...
class MobileMamba(torch.nn.Module):
    def __init__(self, img_size=224,
                 ...
                 drop_path=0., ssm_ratio=1, forward_type="v052d",
                 pretrained=None, freeze_unused_blocks=True):
        super().__init__()
        self.pretrained = pretrained
        self.embed_dim = embed_dim
        self.out_channels = [embed_dim[0] // 4, embed_dim[0] // 2, embed_dim[0], embed_dim[1]]
        ...
        self._init_weights()
        if freeze_unused_blocks:
            self._freeze_unused_blocks()
    ...
    def _freeze_unused_blocks(self):
        self.blocks3.eval()
        for param in self.blocks3.parameters():
    ...
    def ft_head_keywords(self):
        return {'head.weight', 'head.bias'}, 0
    ...
    def forward(self, x):
        ...
        x1 = self.patch_embed[3](x)
        x = self.patch_embed[4](x1)
        x2 = self.patch_embed[5](x)
        x = self.patch_embed[6](x2)
...
CFG_MobileMamba_B2 = CFG_MobileMamba_B1.copy()
CFG_MobileMamba_B4 = CFG_MobileMamba_B1.copy()


def _build_mobilemamba(model_cfg):
    return MobileMamba(**model_cfg)


def mobilemamba_t2():
    return _build_mobilemamba(CFG_MobileMamba_T2)


def mobilemamba_t4():
    return _build_mobilemamba(CFG_MobileMamba_T4)


def mobilemamba_s6():
    return _build_mobilemamba(CFG_MobileMamba_S6)


def mobilemamba_b1():
    return _build_mobilemamba(CFG_MobileMamba_B1)


def mobilemamba_b2():
    return _build_mobilemamba(CFG_MobileMamba_B2)


def mobilemamba_b4():
    return _build_mobilemamba(CFG_MobileMamba_B4)
